...
function demoPhoto(seed: string, kurta: string, bg: string) {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 480">
      ...
      <path d="M246 178c14-70 132-83 154-8 4 15 2 34-4 50-28-42-114-45-150-8-5-10-5-22 0-34z" fill="#262b39"/>
      <path d="M214 440c10-94 67-142 106-142s96 48 106 142H214z" fill="${kurta}"/>
      <path d="M264 310c18 28 94 28 112 0 16 24 25 73 26 130H238c1-57 10-106 26-130z" fill="#f4f8ff" opacity=".72"/>
      <circle cx="290" cy="196" r="7" fill="#262b39"/>
      <circle cx="350" cy="196" r="7" fill="#262b39"/>
      <path d="M294 236c18 16 43 16 58 0" fill="none" stroke="#8d4a36" stroke-width="8" stroke-linecap="round"/>
      <path d="M0 407c118-43 185-32 261 13 74 43 190 43 379-25v85H0z" fill="#123a72" opacity=".12"/>
    </svg>`;
...
export default async function VikasMitraPage() {
  ...
  return (
    <main className="bg-[#f4f8ff]">
      <section className="bg-[linear-gradient(135deg,#123a72_0%,#1b4c8a_55%,#159a56_100%)] px-4 py-14 text-white">
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {allProfiles.map((profile) => (
              <article
                key={profile.id}
                className="overflow-hidden rounded-lg border border-emerald-200 bg-white shadow-soft"
              >
                <div className="relative h-64 bg-emerald-50">
                  {profile.photo ? (
                    ...
                    <div className="grid h-full place-items-center bg-[linear-gradient(135deg,#eaf1fb_0%,#d9e6f7_100%)] text-center">
                      <div>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-3 space-y-2 text-sm font-semibold text-[#33384a]">
                  ...
                  {(profile.occupation || profile.experience) && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {profile.occupation && (
                        <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-extrabold text-maroon">
                          {profile.occupation}
                        </span>
                      )}
                      {profile.experience && (
