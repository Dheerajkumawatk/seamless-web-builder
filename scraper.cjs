...
(async () => {
    ...
    const URL = "https://sec.rajasthan.gov.in/Winner_Runner.aspx?code=CEM&Period=0&DistrictID=33&LocalBodyID=12084";

    const DISTRICT_SELECTOR =
        'select[id$="DistrictDropDownList"]';

    const ULB_SELECTOR =
        'select[id$="PanchayatSamitiDropDownList"]';
    ...
    async function selectAndWait(
        ...
        if (hasPostBack) {
            ...
            try {
                ...
                console.log(
                    "      ⚠️ Navigation timeout, continuing..."
                );
    ...
    try {
        ...
        const duration =
            await findDurationDropdown();


        if (!duration) {

            throw new Error(
                "SEP 2026 dropdown nahi mila."
            );
        }
        ...
        if (!sepValue) {
            ...
            throw new Error(
                "SEP 2026 value nahi mila."
            );
        }


        console.log(
            `📅 SEP 2026 value: ${sepValue}`
        );
        ...
        if (!districtsLoaded) {
            ...
            throw new Error(
                "District dropdown load nahi hua."
            );
        }
        ...
        for (
            ...
            console.log(
                `📍 DISTRICT ${d + 1}/${districts.length}: ${district.text}`
            );
            ...
            for (
                ...
                try {
                    ...
                    if (
                        ...
                        console.log(
                            `      ✅ New rows inserted: ${result.inserted}`
                        );


                        console.log(
                            `      ⏭️ Existing rows ignored: ${result.ignored}`
                        );
        ...
        console.log(
            `🏢 ULBs processed        : ${totalULBs}`
        );

        console.log(
            `📊 Rows scraped          : ${totalRowsFound}`
        );

        console.log(
            `✅ New rows inserted     : ${totalInserted}`
        );

        console.log(
            `⏭️ Existing rows ignored : ${totalIgnored}`
        );

        console.log(
            `❌ Failed ULBs           : ${failedULBs}`
        );
